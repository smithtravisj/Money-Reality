...
for (const file of routeFiles) {
  try {
    ...
    // Replace: export async function GET(...multiline params) {
    // With: export const GET = withRateLimit(async function(...multiline params) {
    content = content.replace(
      /export async function (GET|POST|PATCH|DELETE)\(([\s\S]*?)\) \{/g,
      'export const $1 = withRateLimit(async function($2) {'
    );

    // Handle single-line function signatures (fallback)
    content = content.replace(
