...
/**
 * Generate recurring task instances for a specific pattern
...
export async function generateRecurringInstances(
  ...
  try {
    ...
    if (!pattern) {
      console.log(`[generateRecurringInstances] Pattern ${patternId} not found or inactive`);
      return;
    }
    ...
    const moveToNextOccurrence = (date: Date, recurrenceType: string, daysOfWeek: number[], daysOfMonth: number[]) => {
      ...
      if (recurrenceType === 'weekly' && daysOfWeek.length > 0) {
        ...
        return newDate;
      } else if (recurrenceType === 'biweekly' && daysOfWeek.length > 0) {
        // Find next day that matches selected days of week, but at least 14 days away
        ...
        while (attempts < maxAttempts) {
          newDate.setDate(newDate.getDate() + 1);
          const dayOfWeek = newDate.getDay();
          if (newDate >= minDate && daysOfWeek.includes(dayOfWeek)) {
            return newDate;
          }
          attempts++;
        }
        ...
        // Find next day that matches selected days of month
        let attempts = 0;
        ...
        // Default behavior: add interval days
    ...
    const template = pattern.taskTemplate as any;
    const instances: any[] = [];
    ...
    while (currentDate <= windowEnd) {
      ...
      if (!existingDates.has(instanceDateStr)) {
        // Create due date with the specified time
        const [hours, minutes] = (template.dueTime || '23:59').split(':').map(Number);
    ...
    if (instances.length > 0) {
      console.log(`[generateRecurringInstances] Creating ${instances.length} instances for pattern ${patternId}`);
    ...
    console.error(`[generateRecurringInstances] Error for pattern ${patternId}:`, error);
  }
}
...
export async function generateAllUserRecurringInstances(
  ...
  try {
    ...
    console.error(`[generateAllUserRecurringInstances] Error for user ${userId}:`, error);
  }
}
